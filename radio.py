...
try:
    ...
    for musicFile in musicFiles:
        ...
        if artist == None:
            logger.warning('Missing artist in [{0}]'.format(musicFile))
            artist = '???'
        if title == None:
            logger.warning('Missing Title in [{0}]'.format(musicFile))
            title = '???'
...
class Radio:
    ...
    async def sendPlay(self, message):        
        try:
            embed = discord.Embed(title=":headphones: - Now Playing", description='{0} - {1}'.format(playlist[self.playNow]['artist'], playlist[self.playNow]['title']), color=color['normal'])
            await message.channel.send(embed=embed)
        except:
            await message.channel.send('> :headphones: - Now Playing\n```{0} - {1}```'.format(playlist[self.playNow]['artist'], playlist[self.playNow]['title']))
...
##################################################################################
    if message.content.startswith(";list") or message.content.startswith(";l") or message.content.startswith(";playlist") or message.content.startswith(";pl"):
        ...
        if page > len(playlist):
            ...
            for i in range(page - 1, page + 9):
                try:
                    embed.add_field(name="{0}번 트랙".format(i), value='{0} - {1}'.format(playlist[i]['artist'], playlist[i]['title']), inline=True)
                except:
                    pass
            await message.channel.send(embed=embed)
        return
    ...
    if message.content.startswith(";search") or message.content.startswith(";sh"):
        ...
        try:
            ...
            try:
                ...
                embed.add_field(name="{0}번 트랙".format(search), value='{0} - {1}'.format(artist, title), inline=True)
                await message.channel.send(embed=embed)
                return
            except:
                embed = discord.Embed(title="어..", description='무엇을 찾았어야 하는 걸까요?', color=color['info'])
            ...
            for i in range(0, len(playlist)):
                if searchItem.upper() in str(playlist[i]['artist']).upper() or searchItem.upper() in str(playlist[i]['title']).upper():
                    result += 1
                    embed.add_field(name="{0}번 트랙".format(i), value='{0} - {1}'.format(playlist[i]['artist'], playlist[i]['title']), inline=False)
    ...
    if message.content.startswith(";exit") or message.content.startswith(";e"):
        ...
        try:
            ...
            logger.error('No radioWorker at {0}'.format(message.guild.id))
            embed = discord.Embed(title="?", description='라디오가 꺼저있다는데..', color=color['info'])
            await message.channel.send(embed=embed)
        return
##################################################################################
